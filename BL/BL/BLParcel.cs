...
namespace BL
{
    public sealed partial class BLParcel:IBL
    {
        ...
        public void ConnectParcelToDrone(int droneid)//שייוך חבילה לרחפן
        {
            ...
            try
            {
                ...
                else
                {
                    while (i != 3)
                    {
                        foreach (DO.Parcel item in dl.GetALLParcel())
                        {
                                ...
                                if (item.Priority == (DO.Priorities)i)//אם החבילה היא חירום
                                {
                                    if (IfDronCanTakeParcel(item, droneid))//אם הרחפן יכול לשאת את המשקל
                                    {
                                        ...
                                        if (battary > 0)//אם יש מספיק סוללה כדי להגיע לקחת את החבילה מהשולח
                                        {
                                            ...
                                            {
                                                statin.Lattitude = dl.GetStation(dl.MinFarToStation(targetId).Id).Lattitude;// המיקום של התחנה טעינה הקרובה 
                                                statin.Longitude = dl.GetStation(dl.MinFarToStation(targetId).Id).Longitude;
        ...
        public IEnumerable<BO.ParcelToList> GetALLParcelToList()
        {
            ...
            foreach (DO.Parcel item in dl.GetALLParcel())//מיוי הנתונים ב BL מתוך DAL
            {
        ...
        public void SupplyParcelByDrone(int droneID)// אספקת חבילה על ידי רחפן
        {
            try
            {
                ...
                DO.Parcel parcel = dl.GetALLParcel().ToList().Find(x => x.DroneId == droneID);
                ...
                DO.Customer customer = dl.GetALLCustomers().ToList().Find(x => x.Id == parcel.TargetId);
